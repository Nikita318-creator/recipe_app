...
import 'package:new_recipes/pizza_bloc/models/pizza_model.dart';
...
class PizzasBloc extends Bloc<PizzasEvent, PizzasState> {
  ...
  Future<void> onLoadPizzaCounter(
  ...
  Future<void> onRemovePizza(RemovePizza event, Emitter<PizzasState> emit) async {
    if (state is PizzasLoaded) {
      final state = this.state as PizzasLoaded;
      emit(
